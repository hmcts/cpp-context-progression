...
public class Prompt8Transformer {
    ...
    public Prompt transform(final Prompt prompt,
        ...
        if (nonNull(prompt.getCaseParticipant())) {
            ...
            if (changeWitnessPromptFixListToTxt) {
        ...
        if (nonNull(childrenRefData)) {
            final List<Prompt> transformedChildren = transformChildren(childrenRefData, defendantList);
            transformedPrompt.withChildren(transformedChildren);
        }
        return transformedPrompt.build();
    }

    private List<Prompt> transformChildren(final List<Prompt> children, final List<Defendant> defendants) {
        final List<Prompt> promptList = new ArrayList<>(children);
        ...
        if (isNotEmpty(defendants)) {
            defendants.forEach(defendant -> {
                if (nonNull(defendant.getPersonDefendant())) {
                    ...
                    final Prompt prompt = buildDefendantPrompt(defendant, count, lastElement, children, index, personLabel);
                    ...
                    final Prompt prompt = buildDefendantPrompt(defendant, count, lastElement, children, index, companyLabel);
                    promptList.add(prompt);
                }
            });
        }
    ...
    private Prompt buildDefendantPrompt(final Defendant defendant, AtomicInteger count, final int lastElement, final List<Prompt> children, AtomicInteger index, String label) {

        final List<Prompt> lastChildren = lastElement >= 0 ? children.get(lastElement).getChildren() : null;
        return Prompt.prompt()
                ...
                .withType(OPTION)
                .withChildren(getTransformedChildren(lastChildren, index))

...
import static uk.gov.justice.services.messaging.JsonObjects.createWriter;
...
public class CourtDocumentQueryViewTest {
    ...
    private String objectToString(final Object o) {
        ...
        final JsonWriter jsonWriter = createWriter(sr);
    ...
    private CourtDocument courtDocumentApplicationType(final UUID id, final Integer seqNum, final DocumentTypeRBAC documentTypeRBAC, final UUID documentTypeId) {
        return CourtDocument.courtDocument()
                .withCourtDocumentId(id)
    ...
    private CourtDocument courtDocumentApplicationType(final UUID id, final Integer seqNum, final DocumentTypeRBAC documentTypeRBAC, final List<String> userGroups, final UUID documentTypeId) {
        return CourtDocument.courtDocument()
                .withCourtDocumentId(id)
                .withDocumentCategory(documentCategoryApplicationTemplate())
                .withDocumentTypeId(documentTypeId)
    ...
    private CourtDocument courtDocumentApplicationType(final UUID id, final Integer seqNum, final DocumentTypeRBAC documentTypeRBAC, final List<String> userGroups, final UUID documentTypeId, final String name) {
        return CourtDocument.courtDocument()
                .withCourtDocumentId(id)
                .withDocumentCategory(documentCategoryApplicationTemplate())
                .withDocumentTypeId(documentTypeId)
    ...
    private CourtDocumentEntity courtDocumentEntity(final UUID id, final CourtDocument courtDocument) {
    ...
    private CourtDocumentEntity courtDocumentEntityWithRbac(final UUID id, final CourtDocument courtDocument) {
        final CourtDocumentEntity courtDocumentEntity = new CourtDocumentEntity();
        courtDocumentEntity.setCourtDocumentId(id);
        courtDocumentEntity.setIsRemoved(false);
        courtDocumentEntity.setPayload(objectToString(courtDocument));
    ...
    @Test
    public void shouldFindDocumentsForProsecutionByCaseIdPermitted() throws IOException {
        shouldFindDocumentsForProsecution(true, true, singletonList(randomUUID()), null, null, "Court Clerks", false, true);
    }
    ...
    @Test
    public void shouldFindDocumentsForProsecutionByCaseIdPermittedWhenProsecutingTrue() throws IOException {
        shouldFindDocumentsForProsecution(true, true, singletonList(randomUUID()), null, null, "Court Clerks", true, true);
    }
    ...
    @Test
    public void shouldFindDocumentsForProsecutionByCaseIdsAndApplicationIdPermitted() throws IOException {
        shouldFindDocumentsForProsecution(true, true, asList(UUID.randomUUID(), UUID.randomUUID()), null, asList(UUID.randomUUID(), UUID.randomUUID()), "Court Clerks", false, true);
    }
    ...
    @Test
    public void shouldFindDocumentsForProsecutionByDefendantIdPermitted() throws IOException {
        shouldFindDocumentsForProsecution(true, true, null, UUID.randomUUID(), null, "Court Clerks", false, true);
    }
    ...
    public void shouldNotFindDocumentsByDefendantIdIfDocumentBelongsToOtherDefendant() throws IOException {
        ...
        final JsonArray userGroupArray = createArrayBuilder()
                .add(createObjectBuilder().add("groupName", "Court Clerks").build())
                .build();
    ...
    @Test
    public void shouldFindDocumentsForProsecutionByCaseIdNotPermitted() throws IOException {
        shouldFindDocumentsForProsecution(true, false, asList(UUID.randomUUID()), null, null, "group1", false, false);
    }
    ...
    @Test
    public void shouldNotFindDocumentsForProsecutionByDefendantIdPermitted() throws IOException {
        shouldFindDocumentsForProsecution(false, true, null, UUID.randomUUID(), null, "group1", false, false);
    }
    ...
    private void shouldFindDocumentsForProsecution(final boolean rbackReadPermitted, final boolean permitted, final List<UUID> caseIds, final UUID defendantId, final List<UUID> applicationIds, final String userGroup, final boolean isProsecuting, final boolean shouldStub) throws IOException {
        ...
        final JsonEnvelope jsonEnvelopeOut = target.searchCourtDocumentsForProsecution(jsonEnvelopeIn);
        final CourtDocumentsSearchResult result = jsonObjectToObjectConverter
                .convert(jsonEnvelopeOut.payloadAsJsonObject(), CourtDocumentsSearchResult.class);
    ...
    public void shouldFindDocumentsByApplicationId() throws IOException {
        ...
        final JsonObject applicationJsonObject = createObjectBuilder()
                .add("type", createObjectBuilder().add("code", applicationTypeCode).build())
                .build();
        courtApplicationEntity.setPayload(applicationJsonObject.toString());
        ...
        when(stringToJsonObjectConverter.convert(applicationJsonObject.toString())).thenReturn(applicationJsonObject);
    ...
    @Test
    void shouldHideDocumentWhenApplicationHasConfidentialAddressAndDefenceQuery() throws IOException {
        final String applicationTypeCode = "PL84501";
        ...
        final JsonEnvelope jsonEnvelopeIn = JsonEnvelope.envelopeFrom(
                JsonEnvelope.metadataBuilder().withId(randomUUID())
                        ...
                        .withUserId(randomUUID().toString()).build(),
                jsonObject);
        final List<String> userGroups = singletonList(userGroup);
        final DocumentTypeRBAC documentTypeRBAC = new DocumentTypeRBAC(userGroups, userGroups, now(), now().plusMonths(5), null, null);
        final CourtDocument courtDocument = courtDocumentApplicationType(courtDocumentId, 10, documentTypeRBAC, userGroups, DOCUMENT_TYPE_ID_1, "PostalNotification");
        final CourtDocumentEntity courtDocumentEntity = courtDocumentEntity(courtDocumentId, courtDocument);
        final Set<CourtDocumentIndexEntity> indices = new HashSet<>();
        final CourtDocumentIndexEntity indexEntity= new CourtDocumentIndexEntity();
        indexEntity.setApplicationId(applicationId);
        indices.add(indexEntity);
        courtDocumentEntity.setIndices(indices);
        final UUID applicantId = randomUUID();
        final JsonObject courtDocumentMetadata = objectToJsonObjectConverter.convert(new CourtDocumentMetadata(PostalAddressee.postalAddressee()
                .withCourtApplicationPartyType(CourtApplicationPartyType.PERSON)
                .withCourtApplicationPartyId(applicantId)
                .build()));
        courtDocumentEntity.setDocumentMetadata(courtDocumentMetadata.toString());
        final CourtApplicationEntity courtApplicationEntity = new CourtApplicationEntity();
        final JsonObject applicationJsonObject = createObjectBuilder()
                .add("type", createObjectBuilder().add("code", applicationTypeCode).build())
                .add("applicant", objectToJsonObjectConverter.convert(CourtApplicationParty.courtApplicationParty()
                                .withId(applicantId)
                        .withPersonDetails(Person.person().withIsAddressConfidential(true).build())
                        .build()))
                .build();
        courtApplicationEntity.setPayload(applicationJsonObject.toString());
        when(courtDocumentRepository.findByApplicationIds(Collections.singletonList(applicationId))).thenReturn(asList(courtDocumentEntity));
        when(courtApplicationRepository.findByApplicationId(applicationId)).thenReturn(courtApplicationEntity);
        when(stringToJsonObjectConverter.convert(applicationJsonObject.toString())).thenReturn(applicationJsonObject);
        when(stringToJsonObjectConverter.convert(courtDocumentEntity.getDocumentMetadata())).thenReturn(courtDocumentMetadata);
        when(jsonObjectToObjectConverter.convert(applicationJsonObject, CourtApplication.class)).thenCallRealMethod();
        when(userDetailsLoader.isUserHasPermissionForApplicationTypeCode(any(), any())).thenReturn(true);
        ...
        assertThat(result.getDocumentIndices().isEmpty(), is(true));

    }

    @Test
    void shouldNotHideDocumentWhenApplicationHasNotConfidentialAddressAndDefenceQuery() throws IOException {
        final String applicationTypeCode = "PL84501";
        final String userGroup = "Defence Lawyers";
        final UUID applicationId = UUID.randomUUID();
        final UUID courtDocumentId = UUID.randomUUID();
        ...
        final JsonObject applicationJsonObject = createObjectBuilder()
                ...
                .add("applicant", objectToJsonObjectConverter.convert(CourtApplicationParty.courtApplicationParty()
                        .withId(applicantId)
                        .withPersonDetails(Person.person().withIsAddressConfidential(false).build())
                        .build()))
                .build();
        courtApplicationEntity.setPayload(applicationJsonObject.toString());
        when(courtDocumentRepository.findByApplicationIds(Collections.singletonList(applicationId))).thenReturn(asList(courtDocumentEntity));
        ...
        mockReferenceData(jsonEnvelopeIn, userGroupArray);
        final CourtDocumentIndex.Builder courtDocumentIndexBuilder = CourtDocumentIndex.courtDocumentIndex()
                .withCaseIds(List.of(applicationId))
                .withCategory("Defendant level")
                .withDefendantIds(List.of(randomUUID()))
                .withHearingIds(List.of(randomUUID()))
                .withType("Defendant profile notes");
    ...
    @Test
    void shouldNotHideDocumentWhenApplicationHasConfidentialAddressAndDefenceQueryButNotPostalNotification() throws IOException {
        final String applicationTypeCode = "PL84501";
        final String userGroup = "Defence Lawyers";
        final UUID applicationId = UUID.randomUUID();
        final UUID courtDocumentId = UUID.randomUUID();
        ...
        final List<String> userGroups = singletonList(userGroup);
        final DocumentTypeRBAC documentTypeRBAC = new DocumentTypeRBAC(userGroups, userGroups, now(), now().plusMonths(5), null, null);
        final CourtDocument courtDocument = courtDocumentApplicationType(courtDocumentId, 10, documentTypeRBAC, userGroups, DOCUMENT_TYPE_ID_1, "test");
        final CourtDocumentEntity courtDocumentEntity = courtDocumentEntity(courtDocumentId, courtDocument);
        final Set<CourtDocumentIndexEntity> indices = new HashSet<>();
        final CourtDocumentIndexEntity indexEntity= new CourtDocumentIndexEntity();
        indexEntity.setApplicationId(applicationId);
        ...
        final JsonObject courtDocumentMetadata = objectToJsonObjectConverter.convert(new CourtDocumentMetadata(PostalAddressee.postalAddressee()
                .withCourtApplicationPartyType(CourtApplicationPartyType.PERSON)
                .withCourtApplicationPartyId(subjectId)
                .build()));
        courtDocumentEntity.setDocumentMetadata(courtDocumentMetadata.toString());
        final CourtApplicationEntity courtApplicationEntity = new CourtApplicationEntity();
        final JsonObject applicationJsonObject = createObjectBuilder()
                .add("type", createObjectBuilder().add("code", applicationTypeCode).build())
                .add("subject", objectToJsonObjectConverter.convert(CourtApplicationParty.courtApplicationParty()
                        .withId(subjectId)
                        .withPersonDetails(Person.person().withIsAddressConfidential(false).build())
                        .build()))
                .build();
    ...
    @Test
    void shouldNotHideDocumentWhenApplicationHasConfidentialAddressAndPostalNotificationButNotDefenceQuery() throws IOException {
        final String applicationTypeCode = "PL84501";
    ...
    public void shouldNotFindDocumentsByApplicationIdWhenUserNotAuthoriseToViewDocuments() throws IOException {
        ...
        final CourtApplicationEntity courtApplicationEntity = new CourtApplicationEntity();
        final JsonObject applicationJsonObject = createObjectBuilder()
                .add("type", createObjectBuilder().add("code", "PL84501").build())
                .build();
        courtApplicationEntity.setPayload(applicationJsonObject.toString());
        when(courtApplicationRepository.findByApplicationId(applicationId)).thenReturn(courtApplicationEntity);
        when(stringToJsonObjectConverter.convert(applicationJsonObject.toString())).thenReturn(applicationJsonObject);
        ...
        assertThrows(ForbiddenRequestException.class, () -> target.searchCourtDocuments(jsonEnvelopeIn));
    }
    ...
    public void shouldFindDocumentsIsFilterNonCpsProsecutorRelatedDocumentWhenUserHasExcludeNonCpsProsecutorRole() throws IOException {
        ...
        final UUID nowCourtDocumentId = randomUUID();
        final List userGroups = asList("Court Clerks", "Non CPS Prosecutors");
        final List userGroupsOnlyNonCpsProsecutors = asList("Non CPS Prosecutors");
        final DocumentTypeRBAC documentTypeRBAC1 = new DocumentTypeRBAC(userGroups, userGroups, now(), now().plusMonths(5), userGroups, userGroups);
        final DocumentTypeRBAC documentTypeRBAC2 = new DocumentTypeRBAC(userGroupsOnlyNonCpsProsecutors, userGroupsOnlyNonCpsProsecutors, now(), now().plusMonths(5), userGroupsOnlyNonCpsProsecutors, userGroupsOnlyNonCpsProsecutors);
        final CourtDocument courtDocument1 = courtDocumentApplicationType(nowCourtDocumentId, 20, documentTypeRBAC1, userGroups, DOCUMENT_TYPE_ID_1);
        final CourtDocumentEntity courtDocumentEntity1 = courtDocumentEntityWithRbac(nowCourtDocumentId, courtDocument1);

        final UUID secondNowCourtDocumentId = randomUUID();
        final CourtDocument courtDocument2 = courtDocumentApplicationType(secondNowCourtDocumentId, 10, documentTypeRBAC2, userGroupsOnlyNonCpsProsecutors, DOCUMENT_TYPE_ID_2);
        final CourtDocumentEntity courtDocumentEntity2 = courtDocumentEntityWithRbac(secondNowCourtDocumentId, courtDocument2);
        ...
        mockReferenceData(jsonEnvelopeIn, userGroupArray);
        final JsonObject loggedInUserPermissions = createObjectBuilder().add("permissions",
                        createArrayBuilder()
                                .add(createObjectBuilder().add("object", "CDES_EXCLUDE_NON_CPS_ROLE").build())
                                .build())
                .build();
    ...
    public void shouldApplicationTypeFilterDoesNotWorkWhenApplicationNotFound() throws IOException {
        ...
        courtApplicationEntity.setPayload(applicationJsonObject.toString());
        when(courtDocumentRepository.findByApplicationIds(Collections.singletonList(applicationId))).thenReturn(asList(courtDocumentEntity1, courtDocumentEntity2, courtDocumentEntity));
        when(courtApplicationRepository.findByApplicationId(applicationId)).thenThrow(new NoResultException());
    ...
    public void shouldFindDocumentsIsNotFilterNonCpsProsecutorRelatedDocumentWhenUserHasNotExcludeNonCpsProsecutorRole() throws IOException {
        ...
        final JsonObject loggedInUserPermissions = createObjectBuilder().add("permissions",
                        createArrayBuilder()
                                .add(createObjectBuilder().add("object", "something else").build())
                                .build())
                .build();
        mockLoggedInUserPermissions(loggedInUserPermissions);
    ...
    @Test
    public void shouldFindDocumentsByHearingId() throws IOException {
    ...
    public void shouldFindDocumentsByHearingIdWithRbac() throws IOException {
        ...
        final JsonArray userGroupArray = createArrayBuilder()
                .add(createObjectBuilder().add("groupName", "group1").build())
                .build();
        final JsonObject jsonObject = createObjectBuilder()
                .add(DEFENDANT_ID_PARAMETER, defendantId.toString())
                .add(HEARING_ID_PARAMETER, hearingId.toString())
    ...
    private void mockLoggedInUserPermissions(final JsonObject loggedInUserPermissions) {
        when(requester.request(any(JsonEnvelope.class))).thenAnswer(invocationOnMock -> {
            final JsonEnvelope envelope = (JsonEnvelope) invocationOnMock.getArguments()[0];
            JsonObject responsePayload = null;
            if (envelope.metadata().name().equals("usersgroups.get-logged-in-user-permissions")) {
                responsePayload = loggedInUserPermissions;
            }
            return envelopeFrom(envelope.metadata(), responsePayload);
        });
    }
    ...
    @Test
    public void shouldGetCourtDocumentForProsecutionByCaseIdAndDefedantId() throws IOException {
        shouldFindDocumentsForProsecution(true, true, asList(UUID.randomUUID(), UUID.randomUUID()), UUID.randomUUID(), null, "Legal Advisers", false, true);
    }
}

...
public class DefaultQueryApiMaterialNowsMaterialIdContentResourceTest {
    ...
    public void shouldRunAllInterceptorsAndFetchAndStreamDocument() {
        ...
        final Response documentContentResponse = endpointHandler.getMaterialNowsByMaterialIdContent(materialId.toString(), userId);
    ...
    public void shouldRethrowAnyInterceptorException() {
        ...
        try {
            endpointHandler.getMaterialNowsByMaterialIdContent(materialId.toString(), userId);
    ...
    public void shouldOverrideGeneratedDefaultAdapterClass() {
        assertThat(endpointHandler.getClass().getName(), is("uk.gov.justice.api.resource.DefaultQueryApiMaterialNowsMaterialIdContentResource"));
    }

    private void verifyInterceptorChainExecution() {
        verify(interceptorChainProcessor).process(interceptorContextCaptor.capture());

        assertThat(interceptorContextCaptor.getValue().inputEnvelope(), jsonEnvelope(metadata().withName(PROGRESSION_QUERY_MATERIAL_NOWS_CONTENT).withUserId(userId.toString()),

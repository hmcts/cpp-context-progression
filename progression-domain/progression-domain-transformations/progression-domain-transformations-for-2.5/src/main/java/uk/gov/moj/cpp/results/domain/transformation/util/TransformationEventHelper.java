...
import static uk.gov.moj.cpp.results.domain.transformation.util.ProsecutionCaseHelper.transformProsecutionCase;
...
public class TransformationEventHelper {
    ...
    private static final String LOG_BEFORE="Before -> {}";
    private static final String LOG_AFTER="After -> {}";
    ...
    public JsonEnvelope buildDefendantCaseOffencesTransformedPayload(final JsonEnvelope event, final String newEvent) {
        ...
        if (LOGGER.isInfoEnabled()) {
            LOGGER.info(LOG_BEFORE, payload);
        }
        ...
        if (LOGGER.isInfoEnabled()) {
            LOGGER.info(LOG_AFTER, transformedPayload);
        }

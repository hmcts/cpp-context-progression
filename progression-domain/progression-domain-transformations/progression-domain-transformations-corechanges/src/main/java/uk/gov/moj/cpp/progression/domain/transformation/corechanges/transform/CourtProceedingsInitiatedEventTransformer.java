...
import static uk.gov.moj.cpp.progression.domain.transformation.corechanges.core.SchemaVariableConstants.FIELD_COURT_PROCEEDINGS_INITIATED;
...
import uk.gov.moj.cpp.progression.domain.transformation.corechanges.core.EventTransformationException;
...
public class CourtProceedingsInitiatedEventTransformer implements ProgressionEventTransformer {
    ...
    private static final Logger LOGGER = getLogger(CourtProceedingsInitiatedEventTransformer.class);
    ...
    private Object defendantTransform(final JsonObject jsonObject, final ZonedDateTime courtProceedingsInititated) {
        ...
        for (final Map.Entry<String, JsonValue> property : jsonObject.entrySet()) {
            ...
            if (key.equalsIgnoreCase(FIELD_ID)) {
                if (jsonObject.containsKey(FIELD_COURT_PROCEEDINGS_INITIATED)) {
                    LOGGER.warn("Defendant {} already have {} with value {} ", value, FIELD_COURT_PROCEEDINGS_INITIATED, jsonObject.getString(FIELD_COURT_PROCEEDINGS_INITIATED));
                } else {
                    result.add(FIELD_COURT_PROCEEDINGS_INITIATED, courtProceedingsInititated.toString());
                }
            }
        }
        return result;

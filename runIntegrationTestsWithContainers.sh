...
. $CPP_DOCKER_DIR/docker-utility-functions.sh
...
function healthCheck {
  CONTEXT=("$CONTEXT_NAME-command-handler" "${CONTEXT_NAME}-event-listener" "${CONTEXT_NAME}-event-processor" "${CONTEXT_NAME}-query-view")
...
function runFileServiceLiquibase() {
    ...
    mvn org.apache.maven.plugins:maven-dependency-plugin:2.10:copy -DoutputDirectory=target -Dartifact=uk.gov.justice.services:file-service-liquibase:${FRAMEWORK_LIBRARIES_VERSION}:jar
...
function fullBuildDeployAndTest {
  buildWars
  deployAndTest
}

...
import static uk.gov.moj.cpp.progression.helper.EventSelector.EVENT_SELECTOR_INTERPRETER_UPDATED_FOR_DEFENDANT;
...
public class UpdateInterpreterForDefendantHelper extends AbstractTestHelper {
    ...
    public UpdateInterpreterForDefendantHelper(String caseId, String defendantId) {
        this.caseId = caseId;
    ...
    public void verifyInterpreterForDefendantUpdated() {
        ...
        poll(getDefendantsByCaseId(caseId)).until(status().is(OK),
    ...
    private Matcher<ReadContext> createJsonMatchers(JsonPath jsonRequest) {
        ...
        return allOf(withJsonPath("$.defendants", hasSize(1)),
                        withJsonPath("$.defendants[0].defendantId", is(defendantId)),

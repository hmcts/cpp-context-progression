...
class HearingMapper {
    constructor(hearingJson, courtRegisterFragment, defendant) {
    ...
    isDefendantPresent() {
        if(this.hearingJson.defendantAttendance) {
            ...
            return !!defendantAttendance.attendanceDays.find(a => a.day === this.courtRegisterFragment.registerDate);
        }
        return false;
    }
    ...
    getDefendantAppearanceDetails() {
        if(this.hearingJson.defendantAttendance) {
            const defendantAttendance = this.hearingJson.defendantAttendance.find(d => d.defendantId = this.defendant.id);
            const attendanceDay = defendantAttendance.attendanceDays.find(a => a.day === this.courtRegisterFragment.registerDate);
            if(attendanceDay) {

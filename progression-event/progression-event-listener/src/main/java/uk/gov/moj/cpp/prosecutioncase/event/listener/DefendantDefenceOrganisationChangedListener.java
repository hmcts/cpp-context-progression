...
public class DefendantDefenceOrganisationChangedListener {
    @Inject
    ...
    private static final Logger LOGGER = LoggerFactory.getLogger(DefendantDefenceOrganisationChangedListener.class);

    @Handles("progression.event.defendant-defence-organisation-changed")
    public void processDefendantDefenceOrganisationChanged(final JsonEnvelope event) {
        if (LOGGER.isInfoEnabled()) {
            LOGGER.info("received event progression.event.defendant-defence-organisation-changed {} ", event.toObfuscatedDebugString());
        }
        final DefendantDefenceOrganisationChanged defendantDefenceOrganisationChanged = jsonObjectConverter.convert(event.payloadAsJsonObject(), DefendantDefenceOrganisationChanged.class);
        final UUID prosecutionCaseId = defendantDefenceOrganisationChanged.getProsecutionCaseId();
        final UUID defendantId = defendantDefenceOrganisationChanged.getDefendantId();
        final AssociatedDefenceOrganisation associatedDefenceOrganisation = defendantDefenceOrganisationChanged.getAssociatedDefenceOrganisation();

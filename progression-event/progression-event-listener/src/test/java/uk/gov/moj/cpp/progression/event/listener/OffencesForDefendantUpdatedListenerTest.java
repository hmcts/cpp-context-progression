...
public class OffencesForDefendantUpdatedListenerTest {
    ...
    @Spy
    private OffenceForDefendantUpdatedToEntity converter = new OffenceForDefendantUpdatedToEntity();
    ...
    public void setup() throws NoSuchFieldException, IllegalAccessException {
        ...
        Class<?> aClass = jsonObjectToObjectConverter.getClass().getSuperclass();

        Field objectMapperField = aClass.getDeclaredField("mapper");
    ...
    public void shouldAddOffenceForDefendant() {
        //given
        JsonEnvelope envelope = getJsonEnvelope(defendantId, offence2, "unique wordings", offence3, "wording 3");
    ...
    public void shouldUpdateOffenceForDefendant() {
        Defendant defendantDetail = getDefendantDetail(offence1, "wording 1", offence2, "wordings-pleas");
        //given
        JsonEnvelope envelope = getJsonEnvelope(defendantId, offence2, "wordings", offence3, "wording 3");
        ...
        verify(defendantRepository).save(argumentCaptor.capture());
        assertThat(2,is(argumentCaptor.getValue().getOffences().size()));
        List<UUID> ids=argumentCaptor.getValue().getOffences().stream().map(s->s.getId()).collect(Collectors.toList());
    ...
    public void shouldDeleteOffenceForDefendant() {
        Defendant defendantDetail = getDefendantDetail(offence1, "wording 1", offence2, "wordings-pleas");
        //given
        JsonEnvelope envelope = getJsonEnvelopeForDelete(defendantId, offence2, "wording 1");
        when(defendantRepository.findBy(defendantId)).thenReturn(defendantDetail);
        //when
        listener.updateOffencesForDefendant(envelope);
        //then
    ...
    private Defendant getDefendantDetail(UUID id1, String word1, UUID id2, String word2) {

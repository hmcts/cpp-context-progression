...
public class ProsecutorUpdatedTransformationTest {
    ...
    @Test
    public void shouldTransformProsecutorUpdatedJson() throws IOException {
        final JsonObject specJson = readJsonViaPath("src/transformer/progression.event.case-cps-prosecutor-updated-spec.json");
        assertNotNull(specJson);

        final JsonObject inputJson = readJson("/progression.event.case-cps-prosecutor-updated.json");
        ...
        with(outputCase.toString())
                ...
                .assertThat("$.prosecutingAuthority", equalTo(((JsonString) inputProsecutionCase.read("$.prosecutionAuthorityCode")).getString()))

...
public class ListCourtHearingTransformer {
    ...
    public ListCourtHearing transform(final JsonEnvelope jsonEnvelope, final List<ProsecutionCase> prosecutionCases,
        ...
        final List<HearingListingNeeds> hearingsList = buildHearingListingNeeds(
                jsonEnvelope, prosecutionCases, listHearingRequests, hearingId, isGroupProceedings);
    ...
    public ListCourtHearing transform(final JsonEnvelope jsonEnvelope, final List<ProsecutionCase> prosecutionCases,
        ...
        LOGGER.info("Transforming SPI cases to ListCourtHearing - {} ", hearingListingList);

        final List<HearingListingNeeds> hearingsList = hearingListingList.stream()
                .map(hearingListing -> buildHearingListingNeeds(
                        jsonEnvelope,
                        prosecutionCases,
                        List.of(hearingListing.listHearingRequestList().stream().findFirst().orElseThrow()),
                        hearingListing.hearingId(),
                        isGroupProceedings))
    ...
    protected ZonedDateTime calculateExpectedStartDate(final ListHearingRequest listHearingRequest) {
        ZonedDateTime expectedListingStartDateTime = nonNull(listHearingRequest.getListedStartDateTime()) ? listHearingRequest.getListedStartDateTime() : listHearingRequest.getEarliestStartDateTime();
        if (expectedListingStartDateTime == null && listHearingRequest.getWeekCommencingDate() != null) {
            expectedListingStartDateTime = listHearingRequest.getWeekCommencingDate().getStartDate().atStartOfDay(ZoneOffset.UTC);
        }
        return expectedListingStartDateTime;
    }

    private List<HearingListingNeeds> buildHearingListingNeeds(final JsonEnvelope jsonEnvelope, final List<ProsecutionCase> prosecutionCases, final List<ListHearingRequest> listHearingRequests, final UUID hearingId, final Boolean isGroupProceedings) {
        return listHearingRequests.stream()
                .map(listHearingRequest -> buildHearingListingNeeds(
                        jsonEnvelope, prosecutionCases, hearingId, isGroupProceedings, listHearingRequest))
                .toList();
    }

    private HearingListingNeeds buildHearingListingNeeds(final JsonEnvelope jsonEnvelope, final List<ProsecutionCase> prosecutionCases, final UUID hearingId, final Boolean isGroupProceedings, final ListHearingRequest listHearingRequest) {
    ...
    private boolean calculateIsYouth(final LocalDate birthDate, final ZonedDateTime earliestHearingDate) {
        ...
        return earliestHearingDate.minusYears(18).isBefore(dateOfBirth);
    }
    ...
    List<ListDefendantRequest> listDefendantRequests) {
        ...
        mapOfProsecutionCaseIdWithDefendants.keySet().forEach(prosecutionCaseId -> {
            final ProsecutionCase matchedProsecutionCase = prosecutionCases.stream()
                    .filter(prosecutionCase -> prosecutionCase.getId()
     ...
     * latest
     */
    private LocalDate calculateEarliestHearingDate(final LocalDate noticeDate, final LocalDate referralDate) {

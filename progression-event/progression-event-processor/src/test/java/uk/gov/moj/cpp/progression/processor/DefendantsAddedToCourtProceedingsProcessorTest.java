...
public class DefendantsAddedToCourtProceedingsProcessorTest {
    ...
    @Captor
    private ArgumentCaptor<ExecutionInfo> executeCaptor;
    ...
    @Test
    void shouldRetryCasesReferredToCourtEventMessageWhenProgressionCaseIsNotInViewStoreEmpty() throws Exception {
        final UUID userId = randomUUID();
        //Given
        when(jsonEnvelope.payloadAsJsonObject()).thenReturn(payload);
        when(applicationParameters.getAddDefendantRetryIntervals()).thenReturn("1-5-10-30-60-90-120");
        ...
        verify(executionService, times(1)).executeWith(executeCaptor.capture());
        ...
        assertThat(executeCaptor.getValue().getJobData().getJsonObject("payload").getInt("interval"), is(1));
    }


    @Test
    void shouldRetryReplayedCasesReferredToCourtEventMessageWhenProgressionCaseIsNotInViewStoreEmpty() throws Exception {
        final UUID userId = randomUUID();
        //Given
        when(jsonEnvelope.payloadAsJsonObject()).thenReturn(payload);
        when(applicationParameters.getAddDefendantRetryIntervals()).thenReturn("1-5-10-30-60-90-120");
    ...
    @Test
    void shouldRaiseExceptionCasesReferredToCourtEventMessageWhenProgressionCaseIsNotInViewStoreEmptyAfterReplays() throws Exception {
        final UUID userId = randomUUID();
        //Given
        when(jsonEnvelope.payloadAsJsonObject()).thenReturn(payload);
        when(applicationParameters.getAddDefendantRetryIntervals()).thenReturn("1-5-10-30-60-90-120");
        ...
        ReplayedDefendantsAddedToCourtProceedings replayedDefendantsAddedToCourtProceedings = ReplayedDefendantsAddedToCourtProceedings.replayedDefendantsAddedToCourtProceedings()
                .withDefendants(defendantsAddedToCourtProceedings.getDefendants())
                .withListHearingRequests(defendantsAddedToCourtProceedings.getListHearingRequests())
                .withInterval(7) // after the last retry
                .build();
    ...
    public List<Hearing> createFutureHearings() {
        return singletonList(
                Hearing.hearing()
                        .withHearingDays(
                                singletonList(uk.gov.moj.cpp.progression.listing.domain.HearingDay.hearingDay()

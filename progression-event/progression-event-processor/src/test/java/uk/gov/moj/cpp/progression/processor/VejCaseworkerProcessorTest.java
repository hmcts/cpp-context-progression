...
public class VejCaseworkerProcessorTest {
    ...
    public void shouldProcessVejHearingPopulatedToProbationCaseworker() throws IOException {
        ...
        JsonReader jsonReader = Json.createReader(new StringReader(argumentCaptor));
        JsonObject externalPayload = jsonReader.readObject();
        jsonReader.close();
        final JsonObject hearingObj = (JsonObject) externalPayload.get(HEARING);
        JsonArray policeProsecutionCases = hearingObj.getJsonArray(PROSECUTION_CASES);
        JsonArray hearingProsecutionCases = ((JsonObject) hearing.get("hearing")).getJsonArray(PROSECUTION_CASES);

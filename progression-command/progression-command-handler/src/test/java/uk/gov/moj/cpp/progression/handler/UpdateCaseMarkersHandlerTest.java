...
import uk.gov.justice.core.courts.UpdateCaseMarkers;
...
public class UpdateCaseMarkersHandlerTest {
    ...
    @Spy
    private final Enveloper enveloper = EnveloperFactory.createEnveloperWithEvents(CaseMarkersUpdated.class,
            CaseMarkersUpdatedInHearing.class);
    ...
    @Test
    public void shouldHandleUpdateCaseMarkers() throws EventStreamException {
        final List<Marker> caseMarkers = new ArrayList<>();
        caseMarkers.add(Marker.marker()
                .withId(UUID.randomUUID())
                .build());
        final UpdateCaseMarkers updateCaseMarkers = UpdateCaseMarkers.updateCaseMarkers()
                .withHearingId(UUID.randomUUID())
                .withCaseMarkers(caseMarkers)
                .build();
        final Metadata metadata = Envelope
                .metadataBuilder()
                .withName("progression.command.update-case-markers")
                .withId(randomUUID())
                .build();

        final Envelope<UpdateCaseMarkers> envelope = envelopeFrom(metadata, updateCaseMarkers);
        when(eventSource.getStreamById(any())).thenReturn(eventStream);
        when(aggregateService.get(eventStream, CaseAggregate.class)).thenReturn(caseAggregate);
        updateCaseMarkersHandler.handle(envelope);
        ...
        final JsonEnvelope  resultEnvelope = (JsonEnvelope)envelopes.stream().filter(
                env -> env.metadata().name().equals("progression.event.case-markers-updated")).findFirst().get();
    ...
    @Test
    public void shouldHandleUpdateCaseMarkersToHearing() throws EventStreamException {
        createHearing();
        final List<Marker> caseMarkers = new ArrayList<>();
        caseMarkers.add(Marker.marker()
                .withId(UUID.randomUUID())
        ...
        final Metadata metadata = Envelope
                .metadataBuilder()
                .withName("progression.command.update-case-markers-to-hearing")
                .withId(randomUUID())
                .build();
        ...
        updateCaseMarkersHandler.handleToHearing(envelope);

        final Stream<JsonEnvelope> envelopeStream = verifyAppendAndGetArgumentFrom(eventStream);

        final List<Envelope> envelopes = envelopeStream.map(value -> (Envelope) value).collect(Collectors.toList());
        final JsonEnvelope  resultEnvelope = (JsonEnvelope)envelopes.stream().filter(
                env -> env.metadata().name().equals("progression.event.case-markers-updated-in-hearing")).findFirst().get();
